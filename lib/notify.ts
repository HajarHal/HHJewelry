...
//   EMAIL_FROM       – optional, e.g. "HH Jewelry <orders@yourdomain.com>"
...
function summarize(o: Order): string {
  const lines = [
    ...
    `Total: ${o.total.toLocaleString("en-US")} MAD`,
...
async function sendEmail(o: Order): Promise<void> {
  ...
  const from = process.env.EMAIL_FROM || "HH Jewelry <onboarding@resend.dev>";
  await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from,
      to,
      subject: `New order #${o.id} — ${o.productName}`,
      text: summarize(o),
    }),
  });
}
